...
### Verification note (GOST CMS)
- Added `cmd/examples/verify-cms` — checks CMS signatures.
- Added `cmd/examples/verify-cms-debug` — prints digest/signature permutations for troubleshooting.
- GOST verification now supports CryptoPro XchA (OID `1.2.643.2.2.36.0`): the library tries LE digest and known signature orderings (raw, s||r) to match OpenSSL `cms -verify`.